...
from src.schemas.event import (
    FightListItem, FightDetailResponse, FighterCornerResponse, BroadcastResponse,
)
...
async def live_fights(uow: UnitOfWork = Depends(get_uow)):
...
async def recent_fights(limit: int = Query(50, le=100), uow: UnitOfWork = Depends(get_uow)):
...
async def get_fight(fight_id: str, uow: UnitOfWork = Depends(get_uow)):
    """Fight detail — both competitors, result, round-by-round stats."""
    # Fetch competition
    comp_result = await uow._session.execute(
    ...
    # Fetch competitors
    comps_result = await uow._session.execute(
    ...
    for c in competitors:
        f_result = await uow._session.execute(
    ...
    if comp.event_id:
        ...
        evt_result = await uow._session.execute(
